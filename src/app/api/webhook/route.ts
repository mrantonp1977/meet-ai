...
export async function POST(req: NextRequest) {
  ...
  if (eventType === "call.session_started") {
    ...
    if (!meetingId) {
      return NextResponse.json(
        { error: "Missing meetingId in call.session_started event" },
        { status: 400 }
      );
    }
  ...
  realtimeClient.updateSession({
    ...
    if (!meetingId) {
      return NextResponse.json(
        { error: "Missing meetingId in call.session_participant_left event" },
        { status: 400 }
      );
    }
    ...
    if (!meetingId) {
      return NextResponse.json(
        { error: "Missing meetingId in call.session_ended event" },
        { status: 400 }
      );
    }
